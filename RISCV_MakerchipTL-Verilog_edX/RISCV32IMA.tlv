...
\SV
   ...
   // Loop:
   m4_asm(ADDI, x13, x13, 11)
   m4_asm(MUL, x14, x12, x13)           // Incremental summation
...
\TLV
   ...
   $dec_bits[16:0] = {$funct7, $funct3, $instr[6:0]};
   
   //Check which instruction is being called
   //U-Type instructions
   $is_lui = $dec_bits ==? 17'bx_0110111;
   $is_auipc = $dec_bits ==? 17'bx_0010111;
   //J-Type instruction
   $is_jal = $dec_bits ==? 17'bx_1101111;
   //I-Type instruction
   $is_jalr = $dec_bits ==? 17'bx_000_1100111;
   //B-Type instructions
   $is_beq = $dec_bits ==? 17'bx_000_1100011;
   $is_bne = $dec_bits ==? 17'bx_001_1100011;
   $is_blt = $dec_bits ==? 17'bx_100_1100011;
   $is_bge = $dec_bits ==? 17'bx_101_1100011;
   $is_bltu = $dec_bits ==? 17'bx_110_1100011;
   $is_bgeu = $dec_bits ==? 17'bx_111_1100011;
   //I-Type instructions
   $is_load = $dec_bits ==? 17'bx_0000011;
   //I-Type instructions
   $is_addi = $dec_bits ==? 17'bx_000_0010011;
   $is_slti = $dec_bits ==? 17'bx_010_0010011;
   $is_sltiu = $dec_bits ==? 17'bx_011_0010011;
   $is_xori = $dec_bits ==? 17'bx_100_0010011;
   $is_ori = $dec_bits ==? 17'bx_110_0010011;
   $is_andi = $dec_bits ==? 17'bx_111_0010011;
   $is_slli = $dec_bits == 17'b0_001_0010011;
   $is_srli = $dec_bits == 17'b0_101_0010011;
   $is_srai = $dec_bits == 17'b1_101_0010011;
   //R-Type instructions
   $is_add = $dec_bits == 17'b0000000_000_0110011;
   $is_sub = $dec_bits == 17'b0100000_000_0110011;
   $is_sll = $dec_bits == 17'b0000000_001_0110011;
   $is_slt = $dec_bits == 17'b0000000_010_0110011;
   $is_sltu = $dec_bits == 17'b0000000_011_0110011;
   $is_xor = $dec_bits == 17'b0000000_100_0110011;
   $is_srl = $dec_bits == 17'b0000000_101_0110011;
   ...
   $is_mulhsu = $dec_bits == 17'b0000001_010_0110011;
   $is_div = $dec_bits == 17'b0000001_100_0110011;
   ...
   //If the M-extension ops are being called:
   $mul_rslt[63:0] = ($is_mul || $is_mulhu) ? $src1_value * $src2_value : 64'b0;
   $mulh_rslt[63:0] = $is_mulh ? (($src1_value[31] != $src2_value[31]) ? {1'b1, ($src1_value * $src2_value)} : {1'b0, ($src1_value * $src2_value)}) : 0;
   $mulhsu_rslt[63:0] = $is_mulhsu ? ($src1_value[31] ? {1'b1, ($src1_value * $src2_value)} : {1'b0, ($src1_value * $src2_value)}) : 0;
   //Launch of traps not implemented, set val to 0 if dividend is zero
   ...
   $result[31:0] =
      ...
      $is_mul ? $mul_rslt[31:0] :
      $is_mulh ? $mulh_rslt[63:32] :
      $is_mulhu ? $mul_rslt[63:32] :
      $is_mulhsu ? $mulhsu_rslt[63:32] :
      $is_div ? $src2_value != 32'b0 ? 
         ($src1_value[31] != $src2_value[31] ? {1'b1, $src1_value / $src2_value} : 
            {1'b0, $src1_value / $src2_value}) : 
            32'b0 :
      $is_divu ? $src1_value / $src2_value :
      $is_rem ? ($src1_value[31] != $src2_value[31] ? {1'b1, $src1_value % $src2_value} : 
            {1'b0, $src1_value % $src2_value}) :
      $is_remu ? $src1_value % $src2_value :
         32'b0;
